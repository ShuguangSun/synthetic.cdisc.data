admh_scaff <- rand_per_key("USUBJID", mincount = 0, maxcount = 10, prop_present = 1)
...
admh_lookup <- tribble(~MHBODSYS, ~MHDECOD, ~MHSOC,
                       "cl A", "trm A_1/2", "cl A",
                       "cl A", "trm A_2/2", "cl A",
                       "cl B", "trm B_1/3", "cl B",
                       "cl B", "trm B_2/3", "cl B",
...
#' gen_admh_seq(NULL, x)
#'
#' gen_admh_seq(NULL, data.frame(USUBJID = c('id1', 'id1', 'id2', 'id3', 'id3', 'id3')))
#'
gen_admh_seq <- function(n, .df, ...) {
...
#' gen_admh_mhterm(NULL, x)
#'
gen_admh_mhterm <- function(n, .df, ...) {
...
#' @export
#'
admh_recipe <- tribble(~variables,  ~dependencies, ~func,             ~func_args,                                    ~keep,
                       dtmvars,     dtmdeps,       gen_ase_dtms,      list(study_duration_secs = 1 * secs_per_year), TRUE,
                       mhseqvars,   "USUBJID",     gen_admh_seq,      NULL,                                          TRUE,
